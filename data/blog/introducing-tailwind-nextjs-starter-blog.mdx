...
2. Personalize `metadata.js` (site related information)
...
`data/metadata.js` - contains most of the site related information which should be modified for a user's need.

`data/authors/default.md` - default author information (required). Additional authors can be added as files in `data/authors`.

`data/projects.js` - data used to generate styled card on the projects page.

`data/menu.js` - navigation links.
...
draft (optional)
summary (optional)
images (optional, if none provided defaults to socialBanner in metadata config)
...
Run `tsx ./scripts/compose.ts` to bootstrap a new post.
